...
void ProjectPressureReflect(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  // Assert that input coordinate direction is acceptable
  static_assert(DIR == X1DIR || DIR == X2DIR || DIR == X3DIR, "DIR must be X[123]DIR");
  ...
  pmb->par_for_bndry(
      "ProjectPressureReflect", IndexRange{0, 0}, domain,
      ...
      KOKKOS_LAMBDA(const int &, const int &k, const int &j, const int &i) {
        ...
        const Real mesh_k_energy_density = 0.5 / mesh_density *
                                           (SQR(cons(IM1, mesh_k, mesh_j, mesh_i)) +
                                            SQR(cons(IM2, mesh_k, mesh_j, mesh_i)) +
                                            SQR(cons(IM3, mesh_k, mesh_j, mesh_i)));
        Real mesh_pressure;
        // If magnetic fields are enabled, magnetic energy density must also be accounted
        // for
        Real mesh_mag_energy_density;
        if (fluid == Fluid::glmmhd) {
          mesh_mag_energy_density = 0.5 * (SQR(cons(IB1, mesh_k, mesh_j, mesh_i)) +
                                           SQR(cons(IB2, mesh_k, mesh_j, mesh_i)) +
                                           SQR(cons(IB3, mesh_k, mesh_j, mesh_i)));
          mesh_pressure =
              (gamma - 1.0) * (cons(IEN, mesh_k, mesh_j, mesh_i) - mesh_k_energy_density -
                               mesh_mag_energy_density);
        } else {
          mesh_pressure =
              (gamma - 1.0) * (cons(IEN, mesh_k, mesh_j, mesh_i) - mesh_k_energy_density);
        }
        ...
        // Set energy
        cons(IEN, k, j, i) = ghost_pressure / (gamma - 1.0) + mesh_k_energy_density;
        if (fluid == Fluid::glmmhd) {
          cons(IEN, k, j, i) += mesh_mag_energy_density;
        }
...
template <parthenon::CoordinateDirection DIR, parthenon::BoundaryFunction::BCSide SIDE>
void ProjectPressureOutflow(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  // Assert that input coordinate direction is acceptable
  static_assert(DIR == X1DIR || DIR == X2DIR || DIR == X3DIR, "DIR must be X[123]DIR");
  ...
  pmb->par_for_bndry(
      "ProjectPressureOutflow", IndexRange{0, 0}, domain,
      parthenon::TopologicalElement::CC, coarse, fine,
      KOKKOS_LAMBDA(const int &, const int &k, const int &j, const int &i) {
        // Determine i, j, and k values for a cell in the mesh based on the reference
        // point found above and the input coordinate direction
        const int mesh_i = X1 ? ref : i;
        const int mesh_j = X2 ? ref : j;
        const int mesh_k = X3 ? ref : k;

        // Copy density and momenta
        cons(IDN, k, j, i) = cons(IDN, mesh_k, mesh_j, mesh_i);
        cons(IM1, k, j, i) = cons(IM1, mesh_k, mesh_j, mesh_i);
        cons(IM2, k, j, i) = cons(IM2, mesh_k, mesh_j, mesh_i);
        cons(IM3, k, j, i) = cons(IM3, mesh_k, mesh_j, mesh_i);
        ...
        const Real ghost_pressure = mesh_pressure + (INNER ? -1 : 1) * mesh_density *
                                                        const_accel *
                                                        (2 * num_ghost_cells - 1) * dxf;
        // Set energy density including from magnetic fields if enabled
        cons(IEN, k, j, i) = ghost_pressure / (gamma - 1.0) + mesh_k_energy_density;
        if (fluid == Fluid::glmmhd) {
          cons(IEN, k, j, i) += mesh_mag_energy_density;
        }

        // If magnetic fields are enabled, copy those as well
        if (fluid == Fluid::glmmhd) {
          cons(IB1, k, j, i) = cons(IB1, mesh_k, mesh_j, mesh_i);
          cons(IB2, k, j, i) = cons(IB2, mesh_k, mesh_j, mesh_i);
          cons(IB3, k, j, i) = cons(IB3, mesh_k, mesh_j, mesh_i);
        }
      });
}
